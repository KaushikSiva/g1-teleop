...
show_usage() {
    ...
    echo "  --runtime-profile NAME  Apply preset defaults (LOCAL_MAC, CLOUD_SIM, REMOTE_SIM_BRIDGE)"
...
apply_runtime_profile() {
    ...
    case "$profile" in
        LOCAL_MAC)
            export TELEOP_RUNTIME_PROFILE="LOCAL_MAC"
            ;;
        CLOUD_SIM)
            export TELEOP_RUNTIME_PROFILE="CLOUD_SIM"
            INTERFACE_MODE="sim"
            INPUT_TYPE="zmq_manager"
            OUTPUT_TYPE="all"
            ZMQ_HOST="${QUEST3_ZMQ_HOST:-${TELEOP_ZMQ_HOST:-$ZMQ_HOST}}"
            ZMQ_PORT="${QUEST3_ZMQ_PORT:-${TELEOP_ZMQ_PORT:-$ZMQ_PORT}}"
            ZMQ_OUT_PORT="${QUEST3_ZMQ_OUT_PORT:-${TELEOP_ZMQ_OUT_PORT:-$ZMQ_OUT_PORT}}"
            ;;
        REMOTE_SIM_BRIDGE)
            export TELEOP_RUNTIME_PROFILE="REMOTE_SIM_BRIDGE"
            INPUT_TYPE="zmq_manager"
            OUTPUT_TYPE="all"
            ZMQ_HOST="${QUEST3_ZMQ_HOST:-${TELEOP_ZMQ_HOST:-$ZMQ_HOST}}"
            ZMQ_PORT="${QUEST3_ZMQ_PORT:-${TELEOP_ZMQ_PORT:-$ZMQ_PORT}}"
        ...
        "")
            ;;
        *)
            echo -e "${RED}Error: unknown runtime profile '$profile'${NC}" >&2
            exit 1
            ;;
    esac
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --runtime-profile)
            if [[ -z "$2" ]]; then
                echo -e "${RED}Error: --runtime-profile requires a profile name${NC}" >&2
                exit 1
            fi
            RUNTIME_PROFILE="$2"
            shift 2
...
echo -e "  Runtime Profile:    ${GREEN}$RUNTIME_PROFILE${NC}"
fi
