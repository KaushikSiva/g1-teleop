...
## Quest 3 on Mac + GPU in Cloud

If you want to use a Quest 3 from a Mac but keep MuJoCo and TensorRT on an NVIDIA GPU machine, use the split-runtime workflow in [Mac + Cloud Runtime](../user_guide/mac_cloud_runtime.md).

Recommended layout:

- **Mac**: `bash run_quest3_server.sh`
- **Cloud GPU machine**: `python gear_sonic/scripts/run_sim_loop.py`
- **Cloud GPU machine**: `cd gear_sonic_deploy && bash deploy.sh --runtime-profile CLOUD_SIM --zmq-host <mac-ip> sim`

This keeps MuJoCo and deploy on the same host, which matches the current DDS-based sim transport.
