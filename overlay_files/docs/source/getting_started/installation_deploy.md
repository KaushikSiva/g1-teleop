...
If you are maintaining a smaller team overlay instead of a full fork, add the upstream remote to your working clone:

```bash
git remote add upstream https://github.com/NVlabs/GR00T-WholeBodyControl.git
```

The overlay manifest in `overlay/manifest.yaml` records the pinned upstream ref and the team-owned files that should stay small and easy to reapply.
