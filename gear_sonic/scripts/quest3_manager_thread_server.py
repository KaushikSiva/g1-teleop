...
def run_quest3_manager(
    ...
    #    Y       : start (calibrate + engage policy)
    #    A       : emergency stop
    ...
    mgr_prev_x = False
    mgr_prev_y = False
    last_mode_switch_time = 0.0  # 2-second cooldown between switches

    print("[Manager] Controls: Y = start, A = stop, X = toggle VR 3PT (arms)")
    ...
    try:
        while True:
            ...
            # 2-second cooldown between mode switches
            if now - last_mode_switch_time < 2.0:
                prev_y = y
                prev_a_stop = a
                prev_x_toggle = x_pressed
                continue
            ...
            if current_mode == StreamMode.OFF:
                ...
                if a and not prev_a_stop:
                    new_mode = StreamMode.OFF
                elif x_pressed and not prev_x_toggle:
            ...
            if new_mode != current_mode:
                ...
                if new_mode == StreamMode.OFF:
                    # DON'T send stop to C++ — that would make the robot go limp!
                    # Just exit the Quest 3 manager. The C++ policy keeps running
                    # and the robot stays standing in its current pose.
                    print(f"[Manager] {current_mode.name} -> OFF (Quest 3 manager exiting)")
                    print(f"[Manager] Robot policy still running — robot stays standing")
                    print(f"[Manager] To fully stop: press O in Terminal 2")
                    break
                else:
